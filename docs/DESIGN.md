...
- **Backup format**: primary = raw SQLite file copy; secondary/optional = JSON export for portability. Zip container (backup export, YNAB import) via a hand-rolled reader/writer over system `libz.tbd` (deflate/inflate) — decided over Apple's `Archive`/`Compression` frameworks, which only speak the AAR format, not portable PKZIP; see `AGENTS.md`'s dependency table.
